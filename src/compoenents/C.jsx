import {
    ...
    ListIcon,
    OrderedList,
    UnorderedList, Grid, GridItem, Center, Wrap
...
const courseB=  [
    {     mainH:"C# 9: The Big Picture",
              by:"by Mike Woodring",
              time:"1h 58m ",
              stars:4,
              review:"(259)" },
    { mainH:"C# 9: Getting Started",
          by:"by Paul D. Sheriff",
          time:"11h 42m ",
          stars:4,
          review:"(231)" },
    {  mainH:"Controlling Program Flow in C# 9",
            by:"by Barry Luijbregts",
            time:"43m",
            stars:4,
            review:"(229)" },
    {  mainH:"Introduction to the C# 9 Type System",
            by:"by Jim Cooper",
            time:"11h 45m",
            stars:5,
            review:"(335)" },
    {  mainH:"C# 8 Interfaces",
        ...
        <HStack mt="20px">
            ...
            <VStack w="50%" marginLeft="5%"  marginTop="-60%">
                ...
                <VStack marginBottom="50px" >
                        ...
                        <Heading style={styles.mainH}>C# Development Fundamentals</Heading>
                        
                        
                        <HStack gap="30px"  justifyContent="space-evenly" justifyItems="flex-start"  >
                        ...
                        <Text style={styles.desc}>C# is the most commonly used language for leveraging the .NET Framework. 
                        As such, learning C# is a springboard to creating enterprise systems, desktop applications, websites and mobile applications.
                         The goal of this learning path is to take you from having little to no experience with C# to understanding how to leverage the language's advanced features and how it works on the CLR.</Text>
                </VStack>
                            {/* this part is course select tag */}
                
                        <Menu isOpen={isOpenA} bg="black"   >
                        <Text style={styles.subH2}>Courses in this path</Text>
                            <MenuButton  onClick={isOpenA? onCloseA: onOpenA}  style={styles.courseSelector} >
                                <Flex justifyContent={"space-between"} alignItems="center"> <Text> Introduction to C# </Text> {isOpenA ? <ChevronUpIcon /> : <ChevronDownIcon />}</Flex>  
                            </MenuButton>
                            <MenuList  bg="black"  w="69%" border="none" >
                                <Text style={styles.by}>Dive into C while learning the basics of arrays, collections, and functions as well as advanced topics such as promises and asynchronous programming.</Text>
                                {courseB.map((el,i)=>(<Body key={i} heading={el.mainH} by={el.by} time={el.time} star={el.stars} review={el.review} />))}
                            </MenuList>
                        </Menu>

                        {/* intermediate */}
                        <Menu isOpen={isOpenB} bg="black"   >
                        <Text style={styles.subH2}></Text>
                        <Text style={styles.subH2}></Text>
                            <MenuButton  onClick={isOpenB? onCloseB: onOpenB}  style={styles.courseSelector} >
                                <Flex justifyContent={"space-between"} alignItems="center"> <Text> C# Language Features </Text> {isOpenB ? <ChevronUpIcon /> : <ChevronDownIcon />}</Flex>  
                            </MenuButton>
                            <MenuList  bg="black"  w="69%" border="none" >
                                <Text style={styles.by}>Dive into Angular while learning the basics of arrays, collections, and functions as well as advanced topics such as promises and asynchronous programming.</Text>
                                {courseB.map((el,i)=>(<Body key={i} heading={el.mainH} by={el.by} time={el.time} star={el.stars} review={el.review} />))}
                            </MenuList>
                        </Menu>

                        {/* Advanced  marginTop={isOpenB? "1000px" : 0} */}

                        <Menu isOpen={isOpenC} bg="black"   >
                        <Text style={styles.subH2}></Text>
                        <Text style={styles.subH2}></Text>
                            <MenuButton  onClick={isOpenC? onCloseC: onOpenC}  style={styles.courseSelector} >
                                <Flex justifyContent={"space-between"} alignItems="center"> <Text> Applied C# </Text> {isOpenC ? <ChevronUpIcon /> : <ChevronDownIcon />}</Flex>  
                            </MenuButton>
                            <MenuList  bg="black"  w="69%" border="none" >
                                <Text style={styles.by}>Dive into Angular while learning the basics of arrays, collections, and functions as well as advanced topics such as promises and asynchronous programming.</Text>
                                {courseB.map((el,i)=>(<Body key={i} heading={el.mainH} by={el.by} time={el.time} star={el.stars} review={el.review} />))}
            ...
            <VStack w="50%"  > 
                ...
                <VStack style={styles.rightCard}>
                    ...
                    <UnorderedList >
                            <ListItem>C# syntax</ListItem>
                            <ListItem>Object oriented paradigm</ListItem>
                            <ListItem>Arrays, lists, dictionaries</ListItem>
                            <ListItem>Interfaces</ListItem>
                            <ListItem>Generics</ListItem>
                            <ListItem>Events, delegates, and lambdas</ListItem>
                            <ListItem>LINQ</ListItem>
                            <ListItem>Asynchronous programming</ListItem>
                    </UnorderedList>
                    </VStack>
                    <Spacer />
                    ...
                    <VStack marginLeft="5%" textAlign="justify">
                    <Text style={styles.subH5}>No prior C# knowledge or experience is needed. Some understanding of common programming concepts, such as variables and looping, will be helpful.</Text>
                    </VStack>
                </VStack>
                <Spacer />
                ...
                <Wrap w="70%">
                    <Box style={styles.tworows}>
                        <Text style={styles.tworows.text}>.Net developer</Text>
                    </Box>

                    <Box style={styles.tworows}>
                        <Text style={styles.tworows.text}>web developer</Text>
                    </Box>

                    <Box style={styles.tworows}>
                        <Text style={styles.tworows.text}>Software Engineer</Text>
                    </Box>

                    <Box style={styles.tworows}>
                        <Text style={styles.tworows.text}>ASP.Net</Text>
                    </Box>

                    <Box style={styles.tworows}>
                        <Text style={styles.tworows.text}>Visual Studio</Text>
                    </Box>

                    <Box style={styles.tworows}>
                        <Text style={styles.tworows.text}>WPF</Text>
                    </Box>
                </Wrap>
